...
class AttentionLayer(nn.Module):
    def __init__(self, input_dim, attention_dim = 1):
        super(AttentionLayer, self).__init__()
        self.attention_weights = nn.Parameter(torch.randn(attention_dim, input_dim))
        self.softmax = nn.Softmax(dim=-1)
        
    def forward(self, x):
        # Compute attention scores
        scores = torch.matmul(x, self.attention_weights.T)
...
def visualize_combined_results(true_values, gem_temp, gem_sal, predicted_values, sst_values, ssh_values, max_depth=2000, num_samples=5, limit_depth = 150):
    ...
    for idx in indices:
        ...
        axs[1][1].set_xlabel("Squared Differences (°C²)")
