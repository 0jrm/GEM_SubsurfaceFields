...
class TemperatureSalinityDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path, n_components=15, input_params=None, max_depth = 2000, pred_temp = True, pred_sal = True):
        ...
        # Applying PCA
        self.temp_pcs, self.pca_temp, self.temp_avg, self.temp_std = self._apply_pca(self.TEMP, n_components)
        self.sal_pcs, self.pca_sal, self.sal_avg, self.sal_std = self._apply_pca(self.SAL, n_components)
    ...
    def __getitem__(self, idx):
        ...
        if self.input_params["latcos"]: # use a smaller range - GOM
            # inputs.append(np.cos(2*np.pi*((self.LAT[idx]-18)/24))) # division by double the GoM range
            inputs.append(np.cos(2*np.pi*(self.LAT[idx]/180)))

        if self.input_params["latsin"]: # use a smaller range - GOM
            # inputs.append(np.sin(2*np.pi*(self.LAT[idx]/180)))
            inputs.append(np.sin(2*np.pi*((self.LAT[idx]-18)/24)))  

        if self.input_params["loncos"]: # use a smaller range - GOM
            # inputs.append(np.cos(2*np.pi*((98+self.LON[idx])/36)))  # division by double the GoM range
            inputs.append(np.cos(2*np.pi*(self.LON[idx]/360)))  
            
        if self.input_params["loncos"]: # use a smaller range - GOM
            # inputs.append(np.sin(2*np.pi*((98+self.LON[idx])/36)))
    ...
    def _filter_and_fill_data(self, data, valid_mask):
        ...
        # SSH = data['ADTnoseason_loc'][valid_mask]
    ...
    def _apply_pca(self, data, n_components):
        ...
        pcs = pca.fit_transform(data_standardized.T).T
        avg = scaler.mean_
        std = np.sqrt(scaler.var_)
        return pcs, pca, avg, std
    ...
    def inverse_transform(self, pcs):
        ...
        temp_profiles = temp_profiles * self.temp_std.reshape(-1, 1) + self.temp_avg.reshape(-1, 1)
        sal_profiles = sal_profiles * self.sal_std.reshape(-1, 1) + self.sal_avg.reshape(-1, 1)
...
class AttentionLayer(nn.Module):
    def __init__(self, input_dim, spread = 0.5):
        super(AttentionLayer, self).__init__()
        self.attention_weights = nn.Parameter(torch.rand(input_dim)) # + 2 * spread * torch.rand(input_dim) - spread)
        # self.attention_weights = nn.Parameter(torch.ones(input_dim)) # + 2 * spread * torch.rand(input_dim) - spread)
        self.softmax = nn.Softmax(dim=-1)
        self.all_attention_weights = []  # Store all computed attention weights
        self.last_attention_weights = None  # Store the last computed attention weights
        
    def forward(self, x):
        # Compute attention scores
        scores = x * self.attention_weights
        
        # Apply scaling factor before softmax
        scaling_factor = np.sqrt(x.size(-1))
        scores = scores / scaling_factor
        
        # Apply softmax to get attention weights
        attention_weights = self.softmax(scores)
        self.last_attention_weights = attention_weights.detach().cpu().numpy()  # Store the attention weights
        self.all_attention_weights.append(self.last_attention_weights)  # Store the attention weights
...
class PredictionModel(nn.Module):
    ...
    def __init__(self, input_dim=1, layers_config=[512, 256], output_dim=30, dropout_prob = 0.5):
        ...
        for neurons in layers_config:
            ...
            layers.append(nn.BatchNorm1d(neurons))  # Add batch normalization layer
        ...
        self.attention_layer = AttentionLayer(input_dim) # TODO: choose the best attention_dim (2nd arg)
    ...
    def get_attention_weights(self):
        # Concatenate all attention weights stored during forward passes
        all_attention_weights = np.concatenate(self.attention_layer.all_attention_weights, axis=0)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, epochs=100, patience=10):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 9 or epoch == 0:
            print(f"Epoch [{epoch + 1}/{epochs}] Train Loss: {avg_train_loss:.3f} | Val Loss: {avg_val_loss:.3f} | Patience: {patience - no_improve_count} | Best: {best_val_loss:.3f}")
        ...
        model.attention_weights_history.append(attention_weights)
...
def visualize_combined_results(true_values, gem_temp, gem_sal, predicted_values, sst_values, ssh_values, max_depth=2000, num_samples=5, limit_depth = 150):
    ...
    for idx in indices:
        ...
        plt.suptitle(f"Profile {idx} - SST: {sst_values[idx]:.3f}, SSH (adt): {ssh_values[idx]:.3f}\n"
                     f"T prediction improvement: {accuracy_gain_temp:.3f}%, S prediction improvement: {accuracy_gain_sal:.3f}%", fontsize=16)
...
def plot_relative_errors(true_values, gem_temp, gem_sal, predicted_values, max_depth=2000):
    ...
    axs[0,0].set_title("Temperature error, by depth")
    axs[0,0].set_ylabel("Depth")
    ...
    axs[1,0].set_title("Temperature accuracy gain, by depth")
    axs[1,0].set_ylabel("Depth")
    ...
    axs[0,1].set_title("Salinity error, by depth")
    axs[0,1].set_ylabel("Depth")
    ...
    axs[1,1].set_title("Salinity  accuracy gain, by depth")
    axs[1,1].set_ylabel("Depth")
...
def visualize_attention_weights(model, feature_names):
    attention_weights = model.attention_layer.attention_weights.detach().cpu().numpy()
    plt.figure(figsize=(10, 5))
    plt.barh(feature_names, attention_weights)
...
def analyze_attention_weights(model):
    ...
    print(f'Standard Deviation of Attention Weights: {np.std(attention_weights):.3f}')
    print(f'Max Attention Weight: {np.max(attention_weights):.3f} (Feature: {np.argmax(attention_weights)})')
    print(f'Min Attention Weight: {np.min(attention_weights):.3f} (Feature: {np.argmin(attention_weights)})')


def plot_attention_weights_over_time(attention_weights_history, feature_names):
    plt.figure(figsize=(10, 5))
    for i, feature_name in enumerate(feature_names):
        plt.plot(attention_weights_history[:, i], label=feature_name)
...
if __name__ == "__main__":
    ...
    #fixing confusion with test/val
    val = val_loader
    ...
    ## Un-comment for profile visualizations
    ...
    # TODO: fix attention analysis

    attention_weights_history = np.array(trained_model.attention_weights_history)
    feature_names = [key for key, value in input_params.items() if value]
    # visualize_attention_weights(trained_model, feature_names)
    # analyze_attention_weights(trained_model)
    
    # Normalizing weights 
    # Concatenate all batches to form a single tensor for inputs
    concatenated_inputs = torch.cat([inputs for inputs, _ in train_loader], dim=0)

    # Now calculate the mean and std across all batches for each input parameter
    mean_inputs = torch.mean(concatenated_inputs, dim=0)
    std_inputs = torch.std(concatenated_inputs, dim=0)


    plot_attention_weights_over_time(attention_weights_history * std_inputs.numpy(), feature_names)

    # TODO: fix clusterings
    ...
    # %% TODO: get all results with <0 improvement and plot/map their inputs